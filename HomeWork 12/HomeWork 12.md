...
2. Прямое соединение двух или более таблиц. Создав запрос по таблицам `film`, `film_category`  и `category` можно узнать жанр той или иной кинокартины. Поскольку в таблице `film` много строк, то ограничим получаемый результат запроса:
```
...
4. CROSS JOIN. Поскольку данный вариант соединения выдаёт "на гора" очень много строк, я очень обрадовался, узнав, что пара таблиц, для которых имеется возможность продемонстрировать подобный вариант запроса, содержит всего по две строки:
```
...
5. FULL OUTER JOIN. Поскольку, как я уже говорил ранее, информация в БД отличается полнотой а так же, благодаря внешним ключам, логической целостностью, то для демострации работы внешнего соединения придётся удалить внешние ключи и ограничения NOT NULL, чтобы в некоторые таблицы можно было добавить строки с фиктивной информацией. Поработаем над таблицами `county` и `city`: удалим вышеперчисленные ограничения и добавим в них соответсвенно пару стран, у которых не будет городов и несколько городов, которые не находятся ни в одной стране:
```
