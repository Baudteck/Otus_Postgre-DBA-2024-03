...
1. Посмотрим на показатели, выдаваемые программой `pgbench` при настройках PostgreSQL по умолчанию:
```
...
```
2. Воспользуемся конфигуратором с сайта `https://pgtune.leopard.in.ua`, указав следующие данные:
* версия PG - 15;
* тип OS - Linux;
* тип БД - OLTP;
* объём памяти - 8 ГБ;
...
В ответ получаем параметры настройки:
```
max_connections = 100
shared_buffers = 2GB
...
3. Попробую протестировать базу, используя 1000 клиентов. Удаляю `postgresql.auto.conf`, перезапускаю СУБД и даю нагрузку. Тут же натыкаюсь на ошибку, так как по умолчанию максимальное количество одновременных соединений в 10 раз меньше. Увеличим его до 1000. Да, при такой нагрузке число транзакций в секунду уменьшилось более, чем в 4 раза:
```
...
```
Вновь обратимся к конфигуратору, увеличив максимальное число соединений до 1000:
```
# DB Version: 15
# OS Type: linux
# DB Type: oltp
...
max_parallel_maintenance_workers = 4
```
В ответ он ожидаемо предложил нам в 10 раз уменьшить `"work_mem"`, что вполне логично, иначе тысяче соединений не хватит места в памяти. Однако, запустив тест, я получаю ошибку о нехватке памяти: 
```
$ pgbench -c 1000 -j 8 -P 10 -T 60 "HomeWork 08"
pgbench (15.1 (Ubuntu 15.1-1.pgdg18.04+1))
starting vacuum...end.
...
```
Начали бодро, а закончили совсем плохо: даже общее число транзакций во второй попытке оказалось меньше. У меня только одно предположение - из-за общего увеличения используемой памяти (`shared_buffers` + `max_connections` * `work_mem`) возросли накладные расходы на работу с ней, что не позволило обрабатывать больше транзакций.
...
```
Здесь мы видим прирост около 15-20%, если брать за основу количество выполненных запросов и среднее число транзакций в секунду.