...
```
Для начала мы видим, что процесс 166714 блокирует таблицу `accounts` в режиме `RowExclusiveLock`. Это видно по признаку `granted=true`. Кроме этого он блокирует обычный и виртуальный идентификатор транзакции, являющийся целью блокировки. 
...
- транзакция `"В"` уменьшает счёт №3 и пополняет счёт №1. \
  Ну что же, воспользуемся всё той же таблицей `accounts` и посмотрим, что из этого выйдет. Сначала в каждой сессии начнём транзакцию и сделаем списание средств с соответствующего счёта.\
Сессия №1:
```
HomeWork 07=# SELECT pg_backend_pid();
 pg_backend_pid
